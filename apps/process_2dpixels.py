...
def main():
    ...
    # for csv header
    coord = ['x', 'y']
    header = ['joint' + str(i//2) + '_' + str(coord[i % 2])
    ...
    image_output_parent_path = cfg["image_output"] + dt.strftime(cfg["datetime_format"]) + r'/'
    
    if not os.path.exists(cfg["image_output"] + dt.strftime(cfg["datetime_format"])):
        os.makedirs(cfg["image_output"] + dt.strftime(cfg["datetime_format"]))
    ...
    with open(csvname, 'w', newline='') as f:
        ...
        try:
            ...
            while True:
                ...
                # remove the point without image area.
                removed_xy_pixels = (
                    pixels if is_inshape(width, height, joint_xy_pixels[i]) 
                        else [-1, -1]
                ...
                writer.writerow(list(pixels_iter))
                ...
                img_path = image_output_parent_path + f'{count}.png'
